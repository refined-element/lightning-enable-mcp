...
> **The complete, canonical tool list is the [Tools table in the root README](https://github.com/refined-element/lightning-enable-mcp#tools).** It lists all **15 tools** — **13 free** (out of the box, just a wallet) plus **2 that require `LIGHTNING_ENABLE_API_KEY`** (an [Agentic Commerce subscription](https://lightningenable.com)) — and is pinned to the code by guard tests in both ports, so it never drifts from what the server actually registers. Five of the 15 take an `action` argument; set `LIGHTNING_ENABLE_TOOL_PROFILE=lite` for a 5-tool surface, or `full` to also advertise the pre-consolidation names. The sections below document a **selected subset** in detail; they are not the full inventory.
>
